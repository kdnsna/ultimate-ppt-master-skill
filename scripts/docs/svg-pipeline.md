...
python3 scripts/svg_to_pptx.py <project_path> --trace-conversion
```
...
- `--trace-conversion` writes `logs/conversion_trace_<timestamp>.json` with compact per-slide SVG element conversion diagnostics. Pass a path after the flag to choose a different trace location.
