...
|---|---|---|---|
| `ai` | [`image-generator.md`](./image-generator.md) | Codex/image2 first; `image_gen.py` fallback | `Generated` |
...
`Needs-Manual` is also the entry status for **Offline Manual Mode** (no host-native image tool in use and no `IMAGE_BACKEND` configured). Affected ai rows are marked `Needs-Manual` from the start without a failed attempt — see [`image-generator.md`](./image-generator.md) §7.
...
Executor does NOT invoke Codex/image2, `image_gen.py`, or `image_search.py`.
