...
class DesktopWorkerTest(unittest.TestCase):
    ...
    def test_run_job_creates_web_preview_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(storyboard["deckIRVersion"], "upm-v7")
    ...
    def test_run_job_creates_pptx_preview_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Preview is a rendered SVG cover (title/body from planner), not raw markdown.
            self.assertTrue(result["previewSvg"].startswith("<svg"))
            self.assertIn("<text", result["previewSvg"])
            ...
            self.assertEqual(manifest["schemaVersion"], "v5.2-brief-v1")
            self.assertIn("sourceConfidence", manifest)
            self.assertIn("deliveryScorecard", manifest)
            self.assertIn("referenceStyle", manifest)
            self.assertIn("feedbackLoop", manifest)
            self.assertIn("confirmationBrief", manifest)
            self.assertIn("imageAcceptance", manifest)
            ...
            self.assertEqual(brief["schemaVersion"], "v5.2-brief-v1")
            self.assertIn("sourceConfidence", brief)
            self.assertIn("deliveryScorecard", brief)
            self.assertIn("referenceStyle", brief)
            self.assertIn("feedbackLoop", brief)
            codex_task = (Path(result["projectPath"]) / "codex-task.md").read_text(encoding="utf-8")
            self.assertIn("Expectation Fit and Guided Intake", codex_task)
            self.assertIn("v5.2 Expectation Contract", codex_task)
            self.assertIn("Source confidence", codex_task)
            report = json.loads((Path(result["projectPath"]) / "quality-report.json").read_text(encoding="utf-8"))
            self.assertIn("sourceConfidence", report)
            self.assertIn("deliveryScorecard", report)
            self.assertIn("feedbackLoop", report)
