...
v5.0.0 把 Ultimate PPT Master 重新定位成真实办公 PPT 的交付默认系统。它保留 v4.3 渲染审阅闭环、v4.2 DeckIR 策划包、v4.1 精简网页控制台和 v4.0 混合可编辑契约，再把正常路径变得更直接：默认可编辑 PPTX、一份交付简报、官方/IP 素材边界、Codex-first 视觉、主题艺术方向、微软雅黑排版和 formal-business 审计。
...
- Strategist 现在必须在生成前命名贴合素材的主题艺术方向。文旅、品牌、培训、keynote 和面向公众的材料应把这个概念贯穿首尾页和主标题处理；工作汇报、严肃政务、合规材料可以记录克制例外。
- 交付默认规范明确了微软雅黑、16:9 边距、正文/标题尺度、卡片数量、版式变化和真实 PPT 交付中的反模式。
- formal delivery 审计更明确地检查品牌素材状态、主题艺术方向字段、字体尺度、审美检查、页面角色、版式变化和生成式视觉记录。
...
- 只要材料适合，整套 PPT 应该先有主题级艺术概念。例如文旅材料可以用 `山海交汇 烟火同行` 做首尾页和主标题系统，而不是泛泛的办公风。
