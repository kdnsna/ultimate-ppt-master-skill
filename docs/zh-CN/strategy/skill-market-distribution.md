...
v7.0.0 GitHub 发布与 marketplace 分发是两个独立状态。在 [`v7.0.0` tag](https://github.com/kdnsna/ultimate-ppt-master-skill/pull/15) 发布前，可审查证据为 PR #15。`marketplaceStatus: independent-not-attested` 表示 GitHub Release 既不上传、也不证明市场列表已发布；必须到目标 marketplace 独立查验。
...
- `releaseStatus: unreleased` 与 `marketplaceStatus: independent-not-attested` 是两个独立机器字段，不得用前者推断后者。
