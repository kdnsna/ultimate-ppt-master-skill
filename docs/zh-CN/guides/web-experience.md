...
v4.0.0 继续保持 Handoff 页可执行，并把混合可编辑视觉治理加入生产合同。Bridge 离线时只显示 Bridge 启动命令和 zip 下载；Bridge 在线但还没生成项目时，主动作是发送到 Bridge；handoff 生成后，页面显示本地项目路径、元素生成命令、Agent 命令和 `images/image_prompts.md` 的 `Needs-Manual` fallback 位置。
