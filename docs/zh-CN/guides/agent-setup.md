...
v7.0.0 是下一个候选版本（未发布）。在 [`v7.0.0` tag 与 GitHub Release 页面](https://github.com/kdnsna/ultimate-ppt-master-skill/pull/15) 创建之前，可审查证据以 [PR #15](https://github.com/kdnsna/ultimate-ppt-master-skill/pull/15) 为准；源码元数据不能代替发布证据。GitHub 安装不会自动发布或证明 marketplace 已上架。只想让 Codex 或其他兼容 Agent 使用 Skill，优先走一分钟安装；需要可视化工作台、真实资料解析和本地产物下载，再安装完整仓库。
