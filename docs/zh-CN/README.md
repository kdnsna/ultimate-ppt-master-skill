...
Ultimate PPT Master v4.3 采用分类文档结构。先按你当前要做的事情选择入口。
...
| 需求 | 阅读 |
|---|---|
| 查看最新发布 | [发布说明 - v4.3.0](./release/release-notes-v4.3.0.md) |
...
v4.3 的新增重点是：渲染审阅后生成低风险 `repair-plan.json` 和 `revision-brief.md`，让二次修订可审计、可确认、不自动改事实；v4.2 的 DeckIR 页面地图和 v4.0 的可编辑视觉契约继续作为主线。
