...
The v6.3.8 GitHub release and marketplace distribution are separate states. Verify the GitHub version from the [`v6.3.8` tag and GitHub Release page](https://github.com/kdnsna/ultimate-ppt-master-skill/releases/tag/v6.3.8). `marketplaceStatus: independent-not-attested` means that release neither uploads nor proves a marketplace listing; verify the target marketplace record independently.
