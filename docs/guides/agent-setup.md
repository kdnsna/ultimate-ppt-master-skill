...
The next version is **v7.0.0（unreleased candidate）**. Source metadata is candidate evidence only; until the [`v7.0.0` tag and GitHub Release page](https://github.com/kdnsna/ultimate-ppt-master-skill/pull/15) exist, the authoritative reviewable evidence is [PR #15](https://github.com/kdnsna/ultimate-ppt-master-skill/pull/15). Installing from GitHub does not publish or attest to any marketplace listing.
