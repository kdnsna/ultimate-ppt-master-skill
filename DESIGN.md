...
| Ink | `#171714` | Titles, body text, rules, dark canvas |
...
Use naked structure before containers. A rule, aligned baseline, image edge, or surface change is preferred to a rounded card.
...
- **Decision table**: square corners, quiet rules, explicit criteria, highlighted recommendation.
...
- Three consecutive pages must not share the same light/dark/image surface. An eight-page Web Deck should normally contain at least one dark anchor, one light anchor, and six distinct registered layout families.
...
- Border radius is direction-specific: `0` for precision/evidence, `8-16` for editorial surfaces, and large circular geometry only for the orbital institutional direction.
